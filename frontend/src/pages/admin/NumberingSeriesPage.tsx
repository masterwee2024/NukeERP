...
export default function NumberingSeriesPage() {
  ...
  const [editAssignment, setEditAssignment] = useState<{
    id: string; company_name: string; next_number: number; reset_period: string; updated_at: string;
  } | null>(null);
  ...
  const handleAssign = async (policyId: string, companyId: string) => {
    ...
    const ok = await confirm({ title: "Assign Company", message: `Assign to "${name}"?`, variant: "info", confirmText: "Assign" });
    if (ok) assignMutation.mutate({ pid: policyId, cid: companyId });
  };

  const handleUnassign = async (policyId: string, assignmentId: string, companyName: string) => {
    const ok = await confirm({ title: "Unassign", message: `Remove "${companyName}"?`, variant: "danger", confirmText: "Unassign" });
    if (ok) { unassignMutation.mutate({ pid: policyId, aid: assignmentId }); setEditAssignment(null); }
  };

  const handleEditAssignmentStart = (a: { id: string; company_name: string; next_number: number; reset_period: string; updated_at: string }) => {
    setEditAssignment(a);
  };

  const handleSaveAssignment = async (policyId: string) => {
    if (!editAssignment) return;
    const ok = await confirm({ title: "Update Assignment", message: `Update "${editAssignment.company_name}"?`, variant: "warning", confirmText: "Save" });
    if (!ok) return;
    updateAssignMutation.mutate({
      pid: policyId, aid: editAssignment.id,
      payload: { next_number: editAssignment.next_number, reset_period: editAssignment.reset_period, updated_at: editAssignment.updated_at },
    });
    ...
    <DynamicListDetailPage
      ...
      actionSlots={{
        betweenSections: (record) => {
          const assignments = (record as Record<string, unknown>).company_assignments as Array<{
            id: string; company_id: string; company_name: string; next_number: number; reset_period: string; updated_at: string;
          }> ?? [];
            ...
            return (
              <div className="mt-4 space-y-4 rounded-lg border border-secondary-200 bg-white p-4">
                ...
                <div>
                  ...
                  <input type="number" value={editAssignment.next_number}
                    onChange={(e) => setEditAssignment((p) => p ? { ...p, next_number: parseInt(e.target.value) || 1 } : null)}
                ...
                <div>
                  ...
                  <select value={editAssignment.reset_period}
                    onChange={(e) => setEditAssignment((p) => p ? { ...p, reset_period: e.target.value } : null)}
                    className="mt-1 w-full rounded-lg border border-secondary-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none">
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button onClick={() => handleSaveAssignment(policyId)}
            ...
            <div className="mt-4 space-y-3">
                  ...
                  <div key={a.id} className="rounded-lg border border-secondary-200 bg-white p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => handleUnassign(policyId, a.id, a.company_name)}
              ...
              {unassigned.length > 0 && (
                <select className="w-full rounded-lg border border-secondary-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none"
                  value="" onChange={(e) => { if (e.target.value) handleAssign(policyId, e.target.value); }}>
