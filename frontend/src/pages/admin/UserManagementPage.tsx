...
export default function UserManagementPage() {
  ...
  const deactivateMutation = useMutation({
    mutationFn: async (userId: string) => { await api.post(`/core/admin/users/${userId}/deactivate/`); },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["core/admin/users"] }),
  });
  const reactivateMutation = useMutation({
    mutationFn: async (userId: string) => { await api.post(`/core/admin/users/${userId}/reactivate/`); },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["core/admin/users"] }),
  });
        ...
        detailHeader: (record, refresh) => (
          (record.is_active as boolean) ? (
            <button onClick={async () => {
              const ok = await confirm({ title: "Deactivate", message: `Deactivate ${record.email}?`, variant: "danger", confirmText: "Deactivate" });
              if (ok) { deactivateMutation.mutate(String(record.id)); refresh(); }
            }}
            ...
            <button onClick={async () => {
              const ok = await confirm({ title: "Reactivate", message: `Reactivate ${record.email}?`, variant: "warning", confirmText: "Reactivate" });
              if (ok) { reactivateMutation.mutate(String(record.id)); refresh(); }
            }}
          ...
          <div className="mt-4 space-y-3 rounded-lg border border-secondary-200 bg-white p-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={async () => {
                ...
                const ok = await confirm({ title: "Reset Password", message: `Reset password for ${record.email}?`, variant: "warning", confirmText: "Reset" });
                if (ok) resetPasswordMutation.mutate({ id: String(record.id), password: newPassword });
              }}
